...
class PythonLockTests(unittest.TestCase):
    ...
    def test_python_selector_matching_lock_generator_passes(self):
        with tempfile.TemporaryDirectory() as directory:
            lock = (
            ...
            root = self.fixture(Path(directory), lock)
            (root / ".python-version").write_text("3.11.0\n", encoding="utf-8")
            self.assertEqual([], CHECKER.check_component(root))

    def test_python_selector_mismatching_lock_generator_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            lock = (
                "# This file is autogenerated by pip-compile with Python 3.11\n"
                "demo-pkg==1.2.3 \\\n"
            ...
            root = self.fixture(Path(directory), lock)
            (root / ".python-version").write_text("3.9.13\n", encoding="utf-8")
            findings = CHECKER.check_component(root)
            self.assertTrue(any("selects 3.9" in finding for finding in findings))
