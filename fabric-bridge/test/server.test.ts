...
describe('organization-bound ledger gateway', () => {
  ...
  it('accepts empty strings in optional artifact fields from develop', async () => {
    const command = artifactSubmit();
    command.description = '';
    command.keywords = ['', 'provenance'];
    (command as any).links = [''];
    (command as any).dois = [''];
    (command as any).fundingAgencies = [''];
    (command as any).acknowledgements = '';
    command.contributor = '';
